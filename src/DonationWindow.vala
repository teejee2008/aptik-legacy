...
public class DonationWindow : Dialog {
	...
	public DonationWindow() {
		...
		string msg = _("This application was created for my own use. I'm distributing it in the hope that it will be useful to someone. This is not a commercial product, there's no company behind it and there are no support teams to answer your questions. I work on this application during my free time based on my own requirements and interest.\n\nIf you need help with an issue, or if you have a feature request, please open a ticket in the GitHub issue tracker. If the requested feature is useful to me and I have the time, I will implement it. If it's not useful to me or if it requires more time than I have available, then it will remain in the tracker till someone takes interest and implements it. That's the whole idea behind open-source software. Open-source does not mean \"free\" stuff that you don't need to pay for. It's software that belongs to everyone. If someone needs a feature that is missing, they can take the source code, add the feature, and contribute the change back to the project so that everyone can benefit from their work.\n\nHaving said that, feel free to leave as many feature requests, suggestions and bug reports in the issue tracker as you can. Good ideas are always useful and many of them require little time to implement. Check if an issue was already reported before opening a new item in the tracker, and help out other users if you know the solution to an issue.\n\nYou can leave a donation via PayPal if you wish to say thanks. My PayPal ID is linked below.\n\nThanks,\nTony George");
		label.label = msg;
		label.wrap = true;
		label.xalign = 0.0f;
		label.yalign = 0.0f;
